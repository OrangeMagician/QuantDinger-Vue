...
test('CZSC is a selectable built-in indicator and the legacy trend page keeps multi-period analysis', async () => {
  ...
  assert.doesNotMatch(ide, /class="ide-czsc-toolbar-toggle"/)
  assert.match(ide, /class="ide-toolbar-select ide-toolbar-select--indicator ide-indicator-multiselect-trigger"/)
  ...
  assert.match(ide, /czscMultiPeriodDrawerVisible/)
  assert.match(ide, /czscMultiPeriodTimeframes: \['1d', '30m', '5m'\]/)
  assert.match(ide, /createMultiPeriodRun\(/)
  assert.match(ide, /getTask\(taskId\)/)
  assert.match(ide, /loadCzscMultiPeriodBars/)
  assert.match(ide, /czscMultiPeriodGeneration/)
  assert.match(ide, /this\.czscMultiPeriodDrawerVisible = true/)
  assert.doesNotMatch(ide, /query: \{ symbol: this\.symbol, timeframe: this\.timeframe, mode: 'multi-period' \}/)
